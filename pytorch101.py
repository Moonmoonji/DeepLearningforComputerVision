...
def multiples_of_ten(start, stop):#잘몰겠음
  """
...
def count_negative_entries(x):
  """
  tensor x에 있는 negative한 value의 개수를 구하는 문제 
...
def batched_matrix_multiply(x, y, use_loop=True):
  ...
  bmm함수  : 
   input is a (b \times n \times m)(b×n×m) tensor, mat2 is a 
   (b \times m \times p)(b×m×p) tensor, out will be a
   (b \times n \times p)(b×n×p) tensor.
...
def normalize_columns(x):
  """
  torch.mean함수에서 dim=0 일때는 행들 간의 평균 구하는 거고 -> column normalize 
  torch.mean함수에서 dim=1 일 때는 열등 간의 평균 구함 -> row normalize 
