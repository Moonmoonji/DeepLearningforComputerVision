...
def svm_loss_naive(W, X, y, reg):
  ...
  for i in range(num_train):
    scores = W.t().mv(X[i]) #mv : if input is (nxm), vec is 1-D tensor then out will be 1-D of size. 
    # scores output = (C,)
    correct_class_score = scores[y[i]] #(1,) score에서  정답에 해당하는 correct score만을 따로 빼옴 
...
def svm_loss_vectorized(W, X, y, reg):
  ...
  scores = X.mm(W) #[N,D]x[D,C]=[N,C] 각 training data의 class점수 나타냄 
  correct_class_score = scores[range(num_train),y] #output = N  
  margin = scores - correct_class_score.view(-1,1)+1 # 1 = safety margin

  margin[range(num_train),y] = 0 #정답에 해당하는 클래스는 margin 0으로 설정 
  mask = (margin>0) # margin 0보다 큰거는 mask로 설정 
  loss = margin[mask].sum()/num_train #배열 요소 모두 더하면 loss 
  ...
  mask_correct_y[range(num_train),y]=True #정답인 부분 
  margin[margin>0] = 1 #margin이 0보다 크면 1로 초기화
  margin[margin<0] = 0 #margin이 0보다 작으면 0으로 초기화 
  margin[mask_correct_y] = torch.sum(margin, axis=1)*-1 #다른 클래스에서 1이 나온 횟수만큼 correct label에서 빼줌 
  dW = margin.T.mm(X).T/num_train
...
def sample_batch(X, y, num_train, batch_size):
  ...
  #########################################################################
  # Replace "pass" statement with your code
  indices = torch.randint(num_train,(batch_size,)) #0부터 num_train까지 random하게 integer 생성. (64,)크기로 
  y_batch = y[indices]
  X_batch = X[indices] 
  #########################################################################
  #                       END OF YOUR CODE                                #
...
def softmax_loss_naive(W, X, y, reg):
  ...
  for i in range(num_train):
    scores = W.t().mv(X[i])
    scores = scores-scores.max() #이걸 안 하면 나중에 Loss가 Nan이 되는 부분이 있어서 꼭 해주어야 함 
    ...
    for j in range(num_classes):
      if j == y[i]:
        dW[:,j] += scores_prob[j]* X[i, :]-X[i,:]
      else:
        dW[:,j] += scores_prob[j]* X[i, :]
...
def softmax_get_search_params():
  ...
  ###########################################################################
  # Replace "pass" statement with your code
  learning_rates = np.linspace(1e-2,2e-1,10) #1차원 배열 만들기 
