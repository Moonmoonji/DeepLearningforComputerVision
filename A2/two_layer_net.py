...
def nn_forward_pass(params, X):
    ...
    hidden = X.mm(W1)+b1 #matrix multiplication 
    hidden = hidden.clamp(min=0) #최소가 0이 되도록 그 이하값 교체 
...
def nn_forward_backward(params, X, y=None, reg=0.0):
    ...
    probs[range(num_train), y] -=1  #정답클래스에대해 -1 더함 
  ...
  for reg in regularization_strengths:
    for lr in learning_rates:
      for hs in hidden_sizes:
        ...
        num_iters=3000, batch_size=1000,learning_rate=lr, learning_rate_decay=0.95,
