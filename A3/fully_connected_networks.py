...
class Linear(object):
  ...
  def forward(x, w, b):
    ...
    xx = x.view(x.shape[0],-1) # W와 곱하기 쉽도록 x의 각데이터차원 1차원으로
    out = xx.matmul(w) + b 
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
  ...
  def backward(dout, cache):
    ...
    dx = dout.matmul(w.T).reshape(x.shape) #mul gate
    dw = xx.T.matmul(dout) #mul gate 
    db = dout.sum(axis=0) #add gate. db=dout 
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    return dx, dw, db
...
class ReLU(object):
  ...
  def forward(x):
    ...
    #############################################################################
    # Replace "pass" statement with your code
    out = x.clamp(min = 0) #최소가 0이되도록 그 아랫값 교체 
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    cache = x
  ...
  def backward(dout, cache):
    ...
    #############################################################################
    # Replace "pass" statement with your code
    dx = dout * (x > 0) #x가 양수이면 downstream gradient = dout, 음수이면 0 
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    return dx
...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    loss += self.reg * ( (self.params['W1'] ** 2).sum() + (self.params['W2'] ** 2).sum() ) #l2 regularization 
